...
- **Webflow LCP images specifically:** no native preload toggle exists, so it takes a few steps —
  ...
  4. Alternatively/additionally, set `fetchpriority="high"` as a custom attribute directly on the image element (Element Settings → Custom Attributes) — useful for CMS-driven images where a hardcoded preload URL isn't possible
