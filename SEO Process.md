...
**Platform-specific**
- **Webflow:** custom code embeds in `<head>` are a common silent culprit — audit for missing `defer`/`async`. E.g. Lenis (smooth scroll) should load `defer` and initialize after DOM content is ready, not block initial render; Finsweet Consent Pro is controlled via its own **global/site settings toggle — "Load Script Async"** — use that instead of manually editing `defer`/`async` on the custom code embed. Exact path: **Consent Pro app → Banners → Global Settings → toggle "Load Async" on → Install tab → copy the updated code → paste into the page/site header (replacing the old snippet)**. Verify against Finsweet's own implementation docs before changing — some consent scripts require a specific loading method for compliance reasons, which overrides general performance best practice
- **Webflow LCP images specifically:** no native preload toggle exists, so it takes a few steps —
  1. Select the hero/LCP image in Designer → confirm its loading behavior is not set to lazy (Webflow auto-lazies some above-the-fold images by mistake)
  2. Add `<link rel="preload" as="image" href="..." fetchpriority="high">` via Page Settings → Custom Code → `<head>`, using the exact asset URL from Webflow's asset manager. This preload tag is invisible/separate from the actual `<img>` element — it never affects how the image displays, only when the browser starts fetching it. A single hardcoded `href` (largest variant) is the default recommendation for client builds — simple, low-risk, covers most of the benefit
  3. Full responsive version (`imagesrcset`/`imagesizes` instead of a single `href`) — only worth the extra complexity on high-traffic pages where matching the exact per-device size avoids a double-fetch; not something to standardize into every build. Worst case if `srcset` and preload mismatch is wasted bandwidth (double-fetch) or a DevTools "preload not used" warning — never a visual/layout issue
  4. Set `fetchpriority="high"` as a Custom Attribute directly on the image element too (Element Settings → Custom Attributes), alongside `loading="eager"` — this is required, not optional. PageSpeed checks the preload `<link>` and the `<img>` tag independently; matching `href`/`src` URLs does not link them, so `fetchpriority` set only on the preload `<link>` will still fail the "fetchpriority=high should be applied" audit. It must be set on both tags separately to pass.
  5. Verify via DevTools Network tab (Priority column) or PageSpeed Insights that the image loads early and at high priority
...
|---|---|---|---|---|
| `consentpro.com` runtime JS (Finsweet Consent Pro) | 15.7 KiB | 1,180ms | Script | Toggle **"Load Script Async"** in Finsweet Consent Pro's global/site settings — don't manually edit `defer`/`async` on the custom code embed |
